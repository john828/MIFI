...
class AsymmetricLoss(nn.Module):
    ...
    def forward(self, x, y):
        ...
        x_sigmoid = torch.sigmoid(x)  #二分类使用sigmoid函数。三分类使用softmax()
        xs_pos = x_sigmoid   #正样本
        xs_neg = 1 - x_sigmoid  #负样本
...
class CASL(nn.Module):
    ...
    This loss is intended for single-label classification problems
    '''
    def __init__(self, gamma_pos=0, gamma_neg=4, gamma_hc=0, eps: float = 0.1, reduction='mean', epochs=50,
                 factor=4):
        super(CASL, self).__init__()
...
class CFLoss(nn.Module):
    ...
    This loss is intended for single-label classification problems
    '''
    def __init__(self, gamma_pos=2, gamma_neg=4, gamma_hc=2, eps: float = 0.1, reduction='mean', epochs=50,
                 factor=2):
...
#        eta = abs(1 - self.factor*epoch/(self.epochs-1))
        ...
        # print(xs_pos)
        # print(xs_neg)
        # print(1 - xs_pos - xs_neg)
        # print(self.gamma_pos * targets + self.gamma_neg * anti_targets)
        asymmetric_w = torch.pow(1 - xs_pos,
                                 self.gamma_pos * targets)
        # print(asymmetric_w)
        # print(1 + xs_pos)
        # print(self.gamma_hc * targets)
        positive_w = torch.pow(1 + xs_pos,self.gamma_hc * targets)
        # print(positive_w)
        log_preds = log_preds * ((1 - eta)* asymmetric_w + eta * positive_w)
